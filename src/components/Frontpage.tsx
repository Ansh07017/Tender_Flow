...
export const FrontPage: React.FC<FrontPageProps> = ({
  ...
  return (
    <div className="w-full h-[calc(100vh-100px)] flex gap-6 overflow-hidden text-slate-50 relative">
      ...
      {/* GLOSSY AMBIENT BACKGROUND GLOWS */}
      ...
      <div className="absolute bottom-[-10%] right-[20%] w-[600px] h-[600px] bg-gold-500/10 blur-[150px] rounded-full pointer-events-none mix-blend-screen" />
      ...
      <div className="flex-[8.5] flex flex-col gap-6 h-full relative z-10">
        ...
        <div className="h-[20%] min-h-[120px] bg-slate-900/30 border border-slate-700/50 rounded-3xl p-8 flex items-center justify-between backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
          <div className="space-y-2">
            <h1 className="text-4xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-white via-slate-200 to-gold-400 uppercase drop-shadow-sm">
        ...
        <div className="h-[75%] flex gap-6">
          ...
          <div className="flex-[6.5] bg-slate-900/30 border border-slate-700/50 rounded-3xl p-7 flex flex-col backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] relative overflow-hidden">
            ...
            <div className="flex-1 flex flex-col gap-4 relative z-10 overflow-y-auto pr-2 scrollbar-hide">
              {(isScanning ? [1, 2, 3, 4] : tenders.slice(0, 4)).map((bid, i) => (
                <div 
                  key={i} 
                  className={`flex items-center justify-between p-5 rounded-2xl border transition-all duration-500 backdrop-blur-md
                    ${isScanning 
                  ...
                  {isScanning ? (
                    ...
                    <>
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-3">
                          <span className="text-[9px] font-black px-2.5 py-1 rounded bg-slate-900 border border-slate-700 text-slate-400 shadow-inner">{(bid as Tender).id}</span>
                          ...
                          <span className="text-sm font-bold text-white group-hover:text-gold-400 transition-colors uppercase italic tracking-tight drop-shadow-sm">
                            {(bid as Tender).title}
                          </span>
                        </div>
                        <div className="flex gap-5 text-[10px] font-bold uppercase tracking-widest text-slate-400">
                          <span className="bg-slate-950/50 px-2 py-0.5 rounded">Category: {(bid as Tender).category}</span>
      ...
      <div className="flex-[2] bg-slate-900/30 border border-slate-700/50 rounded-3xl p-8 relative flex flex-col shadow-[0_8px_32px_rgba(0,0,0,0.4)] overflow-hidden min-h-full backdrop-blur-2xl">
        ...
        {activeStep >= 6 && (
          <div className="mt-8 animate-in zoom-in-95 fade-in slide-in-from-bottom-4 duration-700">
            <button 
              onClick={onNavigateToDiscovery} 
              className="w-full bg-gradient-to-r from-gold-500 to-yellow-400 text-slate-950 py-4.5 rounded-xl font-black uppercase text-[11px] tracking-[0.2em] 
                         hover:scale-[1.02] active:scale-95
                         shadow-[0_0_30px_rgba(212,175,55,0.4)] border border-white/40
